...
function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    name: "focus",
    description: "Maintain a durable focus board for the current task: a small note file (default .dsh/focus.md) persisted in this session's workspace that survives compaction and later sessions on the same workspace. Use `set` to pin the current objective and constraints (call it at task start and whenever the objective changes), `append` to log decisions and discoveries, `get` to re-read the board after interruptions or context loss, `clear` to reset it (the old board is archived to .dsh/focus.md.bak). Keep notes terse.",
    parameters: {
      action: {
        type: "string",
        required: true,
      ...
      note: {
        type: "string",
        description: "The note text. Required for `set` (pin/replace the focus) and `append` (add a log entry); ignored for `get`/`clear`.",
      },
    },
    output: {
      schema: {
    ...
    execute: async (args, exec) => {
      ...
      if (action === "set" || action === "append") {
        updated = trimEntries(applyAction(board, action, note, new Date().toISOString()), maxEntries);
